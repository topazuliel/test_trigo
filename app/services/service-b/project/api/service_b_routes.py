...
@service_a_blueprint.route('/prod/service-b/config', methods=['PUT'])
...
@service_a_blueprint.route('/dev/service-b/config', methods=['PUT'])
...
@service_a_blueprint.route('/prod/service-b/config', methods=['POST'])
def insert_config_file():
    data = request.json
    if current_app.mongo.db.Prodconfig.find_one({'conf_name':'service-b'}):
...
@service_a_blueprint.route('/dev/service-b/config', methods=['POST'])
def insert_dev_config_file():
    data = request.json
    if current_app.mongo.db.Devconfig.find_one({'conf_name':'service-b'}):
        return returned_json(status=FAILED, data='', message='config already inserted')
    config = current_app.mongo.db.Devconfig.insert_one(data)
    return returned_json(status=SUCCESS, data='')

@service_a_blueprint.route('/prod/service-b/config', methods=['GET'])
def get_config_file():
    c =[]
    config = current_app.mongo.db.Prodconfig.find_one({'conf_name':'service-b'},{'_id':0})
    if config:
        return returned_json(status=SUCCESS, data=config)
    return returned_json(status=FAILED, message="Can't fined any config", rc=400)

@service_a_blueprint.route('/dev/service-b/config', methods=['GET'])
def get_dev_config_file():
    c =[]
    config = current_app.mongo.db.Devconfig.find_one({'conf_name':'service-b'},{'_id':0})
    if config:
        return returned_json(status=SUCCESS, data=config)
    return returned_json(status=FAILED, message="Can't fined any config", rc=400)
